...
import { Calendar, Clock, FileText, Brain, Eye, Archive, Trash2, CheckCircle } from 'lucide-react';
...
const ExamCard = ({ exam, onUpdate }) => {
  ...
  const handlePublish = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:8000/api/exams/${exam.id}/publish`, {
        method: 'POST',
      });
  ...
  const handleArchive = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:8000/api/exams/${exam.id}/archive`, {
        method: 'POST',
      });
